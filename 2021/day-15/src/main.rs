...
fn main() {
    ...
    println!("min dist to exit : {}", part1(&tile));
    let complete_map:Vec<Vec<u64>> = create_map(&tile);
    println!("min dist to exit : {}", part1(&complete_map));

}

fn part1(input: &Vec<Vec<u64>>) -> u64 {
    let mut dist: Vec<Vec<u64>> = vec![vec![u64::MAX; input[0].len()]; input.len()];
    ...
    while y < max_y {
        while x < max_x {
            ...
            for (ny, nx) in get_neighbours(y, x, max_y, max_x) {
    ...
    dist[max_y - 1][max_x - 1] - input[0][0]
}


fn get_neighbours(y: usize, x: usize, len_y: usize, len_x: usize) -> Vec<(usize, usize)> {
...
mod tests {
    ...
    fn test_parsing_part1() {
        ...
        assert_eq!(part1(&tile), 40);
        assert_eq!(part1(&complete_map), 315);
